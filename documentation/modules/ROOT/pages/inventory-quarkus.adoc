...
Open the newly compiled application by selecting `*'Open in New Tab'*`.
...
[NOTE]
====
If you have the following result on the 'Preview' window, please `*click on the refresh icon of this same window`*, or disable 'https'.
...
It’s time to deploy your service on OpenShift using https://docs.openshift.com/container-platform/4.9/cli_reference/developer_cli_odo/understanding-odo.html[odo^].
...
** ✓  URL inventory: http://inventory-coolstore created**
