...
Everything here is browser based so might have the odd glitch. The easiest resolution to a number of problems is just to refresh the browser, 
or worse case restart the browser. All you work is stored centrally on our servers so you should just be able to re-attach 
...
and re-launching it from the OpenShift Web Console usng the *apps* button in the header bar.
...
The workshop will take a few hours to complete all the way through, if you use the system continuously throughout 
...
to deploy. If your CodeReady deployment tasks fails, chances are you need to re-login to OpenShift from CodeReady. Use the instructions
...
=== My odo push command fails ===

The *odo push* command sometimes fails to deploy reporting failed to *tar -xf*. If this happens the easiest fix is to use a CodeReady terminal window, *cd* to the appropriate directory where your service code is located and then perform *odo delete* and the retry the *odo push* command.
