...
data:
  application.properties: |-
    quarkus.datasource.url=jdbc:mariadb://inventory-mariadb.{USER_ID}-dev.svc:3306/inventorydb
...
----
oc set volume dc/inventory-coolstore --add --configmap-name=inventory --mount-path=/deployments/config -n {USER_ID}-dev #<1> 
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc rsh -n {USER_ID}-dev -c inventory-coolstore dc/inventory-coolstore cat /deployments/config/application.properties
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc rsh -n {USER_ID}-dev dc/inventory-mariadb
----
...
Since you do want Spring Boot to discover the config maps inside the **{USER_ID}-dev** project, you 
need to grant permission to the Spring Boot service account to access the OpenShift REST API and find the 
config maps. 

[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc policy add-role-to-user view -n {USER_ID}-dev -z default
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc delete pod -l deploymentconfig=catalog-coolstore -n {USER_ID}-dev
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc logs -c catalog-coolstore dc/catalog-coolstore -n {USER_ID}-dev | grep hibernate.dialect
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc rsh -n {USER_ID}-dev dc/catalog-postgresql
----
