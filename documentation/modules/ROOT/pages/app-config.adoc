...
IMPORTANT:  This exercise will unfortunately not work in the sandbox environment due to resource constraints. It is still worth reading through it and understanding how ConfigMaps can be used
...
OpenShift provides a mechanism called https://docs.openshift.com/container-platform/4.9/nodes/pods/nodes-pods-configmaps.html[**ConfigMaps**^] 
...
Let's create a *MariaDB* database
for the **Inventory Service** using the MariaDB template that is provided out-of-the-box:

[TIP]
====
https://docs.openshift.com/container-platform/4.9/openshift_images/using-templates.html[OpenShift Templates^] use YAML/JSON to compose 
...
----
oc set volume dc/inventory-coolstore --add --configmap-name=inventory --mount-path=/deployments/config #<1> 
----
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc rsh -c inventory-coolstore dc/inventory-coolstore cat /deployments/config/application.properties
----
...
Since you do want Spring Boot to discover the config maps inside the default project, you 
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
oc logs -c catalog-coolstore dc/catalog-coolstore| grep hibernate.dialect
----
