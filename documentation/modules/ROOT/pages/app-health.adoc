...
Now, `*try to access your http://inventory-coolstore-{USER_ID}-dev.{APPS_HOSTNAME_SUFFIX}[Inventory Service^, role='params-link']*`.
...
A new instance (pod) will be redeployed. Once done, `*try to access again your http://inventory-coolstore-{USER_ID}-dev.{APPS_HOSTNAME_SUFFIX}[Inventory Service^, role='params-link']*`.
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://inventory-coolstore.{USER_ID}-dev.svc:8080/health
----
...
Now, `*access to your http://inventory-coolstore-{USER_ID}-dev.{APPS_HOSTNAME_SUFFIX}[Inventory Service^, role='params-link']*`.
...
for i in {1..60}
do 
    if [ $(curl -s -w "%{http_code}" -o /dev/null http://catalog-coolstore.{USER_ID}-dev.svc:8080/actuator/health) == "200" ]
    then 
...
In your Workspace, `*click on 'Terminal' -> 'Run Task...' ->  'Catalog - Add PodAffinity'*`
...
Now, `*switch back to your Workspace and check the output of the 'Catalog Generate Traffic' task*`.
...
In your Workspace, in the window called **'>_ workshop-tools terminal'**, 
`*execute the following commands*`:

[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://catalog-coolstore.{USER_ID}-dev.svc:8080/actuator/health
----
...
In your Workspace, `*check the output of the 'Catalog Generate Traffic' task*`.
...
In your Workspace, `*click on 'Terminal' -> 'Run Task...' ->  'Probes - Configure Gateway & Web'*`
...
In the OpenShift Web Console, from the **Developer view**,
`*click on 'Monitoring' then select your '{USER_ID}-dev' project*`.
