...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://inventory-coolstore:8080/health
----
...
for i in {1..60}
do 
    if [ $(curl -s -w "%{http_code}" -o /dev/null http://catalog-coolstore:8080/actuator/health) == "200" ]
    then 
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://catalog-coolstore:8080/actuator/health
----
