...
Now, `*try to access your Inventory application test page*`.
...
A new instance (pod) will be redeployed. Once done, `*try to access again your Inventory application*`.
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://inventory-coolstore.${CHE_WORKSPACE_NAMESPACE}-stage.svc:8080/health
----
...
Now, `*access to your Inventory application*`.
...
for i in {1..60}
do 
    if [ $(curl -s -w "%{http_code}" -o /dev/null http://catalog-coolstore.${CHE_WORKSPACE_NAMESPACE}-stage.svc:8080/actuator/health) == "200" ]
    then 
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
curl -w "\n" http://catalog-coolstore.${CHE_WORKSPACE_NAMESPACE}-stage.svc:8080/actuator/health
----
...
In the OpenShift Web Console, from the **Developer view**,
`*click on 'Monitoring' then select your project*`.
